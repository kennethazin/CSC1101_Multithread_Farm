        ...
        try {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (currentTick - lastBreakTick >= ticksUntilNextBreak) {
                    ...
                    Logger.logFarmerAction(currentTick, Thread.currentThread().getId(), id, "started_break", "rest", 0);
                    timeManager.waitTicks(BREAK_DURATION);
                    
                    // Log the end of break
                    currentTick = timeManager.getCurrentTick();
                    Logger.logFarmerAction(currentTick, Thread.currentThread().getId(), id, "finished_break", "rest", 0);
                ...
                Logger.logFarmerCollection(timeManager.getCurrentTick(), Thread.currentThread().getId(), id, waitedTicks, animalCounts);
    ...
    private void stockFields(Map<AnimalType, List<Animal>> collectedAnimals) throws InterruptedException {
        ...
        for (Map.Entry<AnimalType, List<Animal>> entry : sortedAnimals) {
            ...
            try {
                ...
                Logger.logFarmerAction(timeManager.getCurrentTick(), Thread.currentThread().getId(), id, "began_stocking_field", type.toString(), animalsToStock);
                ...
                // Log end of stocking
                Logger.logFarmerAction(timeManager.getCurrentTick(), Thread.currentThread().getId(), id, "finished_stocking_field", type.toString(), stockedCount);
